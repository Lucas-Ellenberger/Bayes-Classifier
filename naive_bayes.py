...
# Student Name: Lucas Ellenberger
# UW Email    : lmellenb@ucsc.edu
...
class NaiveBayes():
    ...
    def fit(self, train_hams:list, train_spams:list):
        ...
        def get_counts(filenames:list):
            ...
            """
            word_dict = dict()
            for file in filenames: # loop through every file
                words = set()
                words = self.word_set(file)
                for w in words: # loop through every word in the file
                   if w in word_dict: # if its in the dictionary, add one to the count
                    word_dict[w] += 1
                   else: # else, add the word to the dict and set its count to 1
                    word_dict[w] = 1
            return word_dict
    ...
    def predict(self, filename:str):
        ...
        prob_ham = np.log(self.num_train_hams / (self.num_train_hams + self.num_train_spams))
        prob_spam = np.log(self.num_train_spams / (self.num_train_hams + self.num_train_spams))
        for w in words:
            prob_ham += np.log((self.word_counts_ham.get(w, 0) + 1) / (self.num_train_hams+ 2))
            prob_spam += np.log((self.word_counts_spam.get(w, 0) + 1) / (self.num_train_spams+ 2))
